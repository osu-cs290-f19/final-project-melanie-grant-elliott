...
map.addEventListener('click', (event) => {
    ...
    console.log("you clicked on latlong: " + event.latlng);
    let addNewMarker = tomtom.L.marker(event.latlng, {icon: catIcon});
    recentlatlng = event.latlng;
...
async function createNewCat(lat, long, color, energy, sociability){
    ...
    console.log("createnewcat latlong: " + lat + " " + long);
...
async function handleAcceptButtonClick(event){
  if(colorInp.value == "" || energyInp.value ==""|| kindnessInp.value ==""|| photoInp.value ==""){
    window.alert("fill out all fields please!");
  }
  else{
    console.log("recentlatlng:" + recentlatlng);
    await createNewCat(recentlatlng.lat.toFixed(6),recentlatlng.lng.toFixed(6),colorInp.value,energyInp.value,kindnessInp.value);
